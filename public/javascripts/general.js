...
var _rollbarConfig = {
            accessToken: "d93c2857de084eb48bd824a0406fddfc",
        ...
        // End Rollbar Snippet
        function sendRollbarError(message){
    ...
    mixpanel.init("3c5e27b8236517aeef1ea053b95a559a");

function onStartList() {
            var date = displayDate();
            var today = date[0]+", "+date[1]+" "+date[2]+", "+date[3];
...
function deleteHabit(element){
        ...
        function displayDate() {
            ...
            today = new Array(days[now.getDay()],
                     months[now.getMonth()],
                     date,
        ...
        function reminder(day) {
            ...
            habitsRef.once("value", function(snapshot) {
                snapshot.forEach(function(childSnapshot){
                    ...
                    var days = childSnapshot.val().weeklyfrequency;
                    
                    if (days[day] && progress < goal){
                        console.log(childSnapshot.val());
                        notifyMe(childSnapshot.val());
                    }
                });
            });
        }

        function notifyMe(habit) {
            //var noti = new Notification(habit.title);
            //console.log(noti);
            if (!("Notification" in window)) {
                ...
                var notification = new Notification(habit.title, options);
                notification.requireInteraction;
                alert("granted");
                //notification.onclick(highlight(habit));
            }
